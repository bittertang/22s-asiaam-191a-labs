...
function addMarker(data){
    ...
    if(data['Is your English your first language?'] == "Yes"){
        ...
        englishFirst.addLayer(L.circleMarker([data.lat,data.lng]).bindPopup(`<h2>Speak English fluently</h2>`))
        createButtons(data.lat,data.lng,data.Location)
        }
    else{
        circleOptions.fillColor = "blue"
        nonEnglishFirst.addLayer(L.circleMarker([data.lat,data.lng]).bindPopup(`<h2>Speak other languages</h2>`))
        createButtons(data.lat,data.lng,data.Location)
    }
    return data
}
...
function processData(results){
    ...
    let allLayers = L.featureGroup([englishFirst,nonEnglishFirst]); // array of both englishFirst and nonEnglishFirst
